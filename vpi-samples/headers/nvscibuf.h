 ...
 * - Applications need to pass attribute values in the valid range when setting attributes
 * in an NvSciBufAttrList.
 * - Applications need to ensure that any non-NULL NvSciBuf structures, such as
 * NvSciBufModule, NvSciBufAttrList, NvSciBufObj, and NvSciBuf AttrList/Object, transport descriptors
 * passed as an input parameter to NvSciBuf APIs were returned as an output parameter by a prior successful
 * invocation of an NvSciBuf API and those structures were not freed.
 *
 ...
 * @implements{18840108}
 */
static const uint32_t NvSciBufMinorVersion = 12U;
...
typedef enum {
     ...
     * NvSciBufImageAttrKey_PlaneColorFormat.
     *
     * Note, in the case of right-justified Bayer RAW 10-bit packed formats,
     * and right-justified Bayer RAW 12-bit packed formats,
     * the value of this attribute is set to the number of bits per pixel,
     * but it does not represent the immediate number of bits to the next pixel
     * due to additional padding bits added for alignment.
     *
     * For example, a color format with 10 bits packed would have 10 bits
     * per pixel, but one uint32_t would contain 3 pixels and 2 bits of padding.
     * Indeed, if the resolution is 1920x1080, the pitch would be 2560 bytes.
     * (1920 pixel / (3 pixels per 4 bytes) = 2560 bytes).
     *
     * Similarly, a color format with 12 bits packed would have 12 bits
     * per pixel, but one uint32_t would contain 2 complete pixels and 1 partial pixel.
     * Indeed, if the resolution is 1920x1080, the pitch would be 2880 bytes.
     * (1920 pixel / (8 pixels per 12 bytes) = 2880 bytes).
     *
     * During validation of reconciled NvSciBufAttrList against input
     * unreconciled NvSciBufAttrList(s), validation fails if reconciliation of
     * NvSciBufGeneralAttrKey_Types attribute from the input unreconciled
     ...
     *
     * Note, that this does not apply for right-justified Bayer RAW 10-bit
     * packed formats. This is because the byte calculation no longer holds.
     * For example, suppose RAW10 packed plane 2560x1984. Using the formula,
     * Plane pitch would be derived as 2560 * (10/8) = 3200. This is incorrect
     * as it fails to account for padding bits.
     *
     * NvSciBufImageAttrKey_PlanePitch =
     *     ((NvSciBufImageAttrKey_PlaneWidth + 2) / 3 pixels ) * 4 bytes
     *
     * Continuing the above example, plane pitch would be derived as
     * ((2560 + 2) / 3) * 4 = 3416. This is a result of the format where
     * every 3 pixels are packed into 4 byte containers. Moreover, to account
     * for the case where division leads to rounding down, we add 2 in the
     * worst case to account for alignment.
     *
     * Similarly, for Bayer RAW 12-bit packed formats:
     *
     * NvSciBufImageAttrKey_PlanePitch =
     *     ((NvSciBufImageAttrKey_PlaneWidth + 1) / 2 pixels ) * 3 bytes
     *
...
typedef enum {
     ...
     */
    NvSciColor_V8_U8,
    /** 10 bit UV components with 6 bits padding for each component */
    NvSciColor_U10V10,
    /** 10 bit VU components with 6 bits padding for each component */
    NvSciColor_V10U10,
    /** 12 bit UV components with 4 bits padding for each component */
    NvSciColor_U12V12,
    /** 12 bit VU components with 4 bits padding for each component */
    NvSciColor_V12U12,
    NvSciColor_U16V16,
    NvSciColor_V16U16,
    ...
    /**
     * Packed YUV formats */
    NvSciColor_Y10_U10__Y10_V10,
    NvSciColor_Y10_V10__Y10_U10,
    NvSciColor_U10_Y10__V10_Y10,
    NvSciColor_V10_Y10__U10_Y10,
    ...
    /** Tightly packed YUV formats */

    /** 10 bit Y component, tightly packed with no padding */
    NvSciColor_Y10t,
    /** 10 bit U component, tightly packed with no padding */
    NvSciColor_U10t,
    /** 10 bit V component, tightly packed with no padding */
    NvSciColor_V10t,

    /** 10 bit UV components, tightly packed with no padding */
    NvSciColor_U10V10t,
    /** 10 bit VU components, tightly packed with no padding */
    NvSciColor_V10U10t,

    /* Right-justified Bayer RAW format 10-bit Packed */
    /* Bit ordering for little endian machine is as follows
     * for NvSciColor_X2Rc10Rb10Ra10_Bayer10RGGB
     * pattern  **    GGGGGGGGGG RRRRRRRRRR BBBBBBBBBB   **    GGGGGGGGGG GGGGGGGGGG RRRRRRRRRR
     *          ++    +--------+ +--------+ +--------+   ++    +--------+ +--------+ +--------+
     * bit      63/62 61      52 51      42 41      32   31/30 29      20 19      10 9        0
     *                                      +--------+         +--------+ +--------+ +--------+
     * pixel                                    3                  2          1          0
     *                                      +-------------------------------------------------+
     * pitch                                                         0
     ...
     * '*' means undefined bit value
     *
     * Notice the differences in bits per pixel. Every 10 bits is a pixel.
     * However, this means that the pitch is no longer a function of the bits per pixel since
     * the 32 bit container also has 2 bits of padding.
     */
    NvSciColor_X2Rc10Rb10Ra10_Bayer10RGGB,
    NvSciColor_X2Rc10Rb10Ra10_Bayer10BGGR,
    NvSciColor_X2Rc10Rb10Ra10_Bayer10GRBG,
    NvSciColor_X2Rc10Rb10Ra10_Bayer10GBRG,
    NvSciColor_X2Rc10Rb10Ra10_Bayer10CRBC,
    NvSciColor_X2Rc10Rb10Ra10_Bayer10CBRC,
    NvSciColor_X2Rc10Rb10Ra10_Bayer10RCCB,
    NvSciColor_X2Rc10Rb10Ra10_Bayer10BCCR,

    /* Right-justified Bayer RAW format 12-bit Packed */
    /* Bit ordering for little endian machine is as follows
     * for NvSciColor_Bayer12RGGB
     * pattern  RRRRRRRRRRRR BBBBBBBBBBBB GGGG     GGGGGGGG GGGGGGGGGGGG RRRRRRRRRRRR
     *          +----------+ +----------+ +--+     +------+ +----------+ +----------+
     * bit      59        48 47        36 35 32    31    24 23        12 11         0
     *                       +----------+ +--+     +------+ +----------+ +----------+
     * pixel                      3                   2          1            0
     *                       +------------------------------------------------------+
     * pitch                                          0
     *
     * Notice the differences in bits per pixel. Every 12 bits is a pixel.
     * However, this means that the pitch is no longer a function of the bits per pixel since
     * the 32 bit container has 2 complete pixels and 1 partial pixel. This pattern continues,
     * until every 12 bytes, at which point 8 pixels are aligned.
     */
    NvSciColor_Bayer12RGGB,
    NvSciColor_Bayer12BGGR,
    NvSciColor_Bayer12GRBG,
    NvSciColor_Bayer12GBRG,
    NvSciColor_Bayer12CRBC,
    NvSciColor_Bayer12CBRC,
    NvSciColor_Bayer12RCCB,
    NvSciColor_Bayer12BCCR,
 ...
 *
 * NvSciBufAttrValColorFmt to NvSciBufAttrValColorStd mapping
 *
 * <pre>
 * +---------------------------------------+---------------------------+
 * | NvSciBufAttrValColorFmt               | NvSciBufAttrValColorStd   |
 * +=======================================+===========================+
 * | NvSciColor_Bayer8RGGB                 | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Bayer8CCCC                 | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Bayer8BGGR                 | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Bayer8GBRG                 | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Bayer8GRBG                 | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Bayer16BGGR                | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Bayer16CCCC                | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Bayer16GBRG                | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Bayer16GRBG                | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Bayer16RGGB                | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Bayer16RCCB                | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Bayer16BCCR                | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Bayer16CRBC                | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Bayer16CBRC                | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Bayer16RCCC                | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Bayer16CCCR                | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Bayer16CRCC                | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Bayer16CCRC                | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X2Bayer14GBRG              | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X4Bayer12GBRG              | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X4Bayer12RCCB              | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X4Bayer12BCCR              | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X4Bayer12CRBC              | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X4Bayer12CBRC              | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X4Bayer12RCCC              | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X4Bayer12CCCR              | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X4Bayer12CRCC              | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X4Bayer12CCRC              | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X6Bayer10GBRG              | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X2Bayer14GRBG              | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X4Bayer12GRBG              | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X6Bayer10GRBG              | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X2Bayer14BGGR              | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X4Bayer12BGGR              | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X6Bayer10BGGR              | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X2Bayer14RGGB              | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X4Bayer12RGGB              | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X4Bayer12RGGB_RJ           | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X6Bayer10RGGB              | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X2Bayer14CCCC              | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X4Bayer12CCCC              | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X6Bayer10CCCC              | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Signed_X2Bayer14CCCC       | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Signed_X4Bayer12CCCC       | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Signed_X12Bayer20CCCC      | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Signed_X6Bayer10CCCC       | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Signed_Bayer16CCCC         | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_FloatISP_Bayer16CCCC       | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_FloatISP_Bayer16RGGB       | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_FloatISP_Bayer16BGGR       | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_FloatISP_Bayer16GRBG       | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_FloatISP_Bayer16GBRG       | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_FloatISP_Bayer16RCCB       | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_FloatISP_Bayer16BCCR       | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_FloatISP_Bayer16CRBC       | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_FloatISP_Bayer16CBRC       | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_FloatISP_Bayer16RCCC       | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_FloatISP_Bayer16CCCR       | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_FloatISP_Bayer16CRCC       | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_FloatISP_Bayer16CCRC       | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X12Bayer20CCCC             | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X12Bayer20BGGR             | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X12Bayer20RGGB             | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X12Bayer20GRBG             | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X12Bayer20GBRG             | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X12Bayer20RCCB             | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X12Bayer20BCCR             | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X12Bayer20CRBC             | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X12Bayer20CBRC             | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X12Bayer20RCCC             | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X12Bayer20CCCR             | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X12Bayer20CRCC             | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X12Bayer20CCRC             | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_U8V8                       | NvSciColorStd_REC601_SR   |
 * |                                       | NvSciColorStd_REC601_ER   |
 * |                                       | NvSciColorStd_REC709_SR   |
 * |                                       | NvSciColorStd_REC709_ER   |
 * |                                       | NvSciColorStd_REC2020_SR  |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_U8_V8                      | NvSciColorStd_REC601_ER   |
 * |                                       | NvSciColorStd_REC709_SR   |
 * |                                       | NvSciColorStd_REC709_ER   |
 * |                                       | NvSciColorStd_REC2020_SR  |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_V8U8                       | NvSciColorStd_REC601_SR   |
 * |                                       | NvSciColorStd_REC601_ER   |
 * |                                       | NvSciColorStd_REC709_SR   |
 * |                                       | NvSciColorStd_REC709_ER   |
 * |                                       | NvSciColorStd_REC2020_SR  |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_V8_U8                      | NvSciColorStd_REC601_ER   |
 * |                                       | NvSciColorStd_REC709_SR   |
 * |                                       | NvSciColorStd_REC709_ER   |
 * |                                       | NvSciColorStd_REC2020_SR  |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_U10V10                     | NvSciColorStd_REC601_SR   |
 * |                                       | NvSciColorStd_REC601_ER   |
 * |                                       | NvSciColorStd_REC709_SR   |
 * |                                       | NvSciColorStd_REC709_ER   |
 * |                                       | NvSciColorStd_REC2020_SR  |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_V10U10                     | NvSciColorStd_REC601_SR   |
 * |                                       | NvSciColorStd_REC601_ER   |
 * |                                       | NvSciColorStd_REC709_SR   |
 * |                                       | NvSciColorStd_REC709_ER   |
 * |                                       | NvSciColorStd_REC2020_SR  |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_U12V12                     | NvSciColorStd_REC601_SR   |
 * |                                       | NvSciColorStd_REC601_ER   |
 * |                                       | NvSciColorStd_REC709_SR   |
 * |                                       | NvSciColorStd_REC709_ER   |
 * |                                       | NvSciColorStd_REC2020_SR  |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_V12U12                     | NvSciColorStd_REC601_SR   |
 * |                                       | NvSciColorStd_REC601_ER   |
 * |                                       | NvSciColorStd_REC709_SR   |
 * |                                       | NvSciColorStd_REC709_ER   |
 * |                                       | NvSciColorStd_REC2020_SR  |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_U16V16                     | NvSciColorStd_REC601_SR   |
 * |                                       | NvSciColorStd_REC2020_ER  |
 * |                                       | NvSciColorStd_REC709_ER   |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_V16U16                     | NvSciColorStd_REC601_SR   |
 * |                                       | NvSciColorStd_REC2020_ER  |
 * |                                       | NvSciColorStd_REC709_ER   |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Y8                         | NvSciColorStd_REC601_SR   |
 * |                                       | NvSciColorStd_REC601_ER   |
 * |                                       | NvSciColorStd_REC709_SR   |
 * |                                       | NvSciColorStd_REC709_ER   |
 * |                                       | NvSciColorStd_REC2020_SR  |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Y10                        | NvSciColorStd_REC601_SR   |
 * |                                       | NvSciColorStd_REC601_ER   |
 * |                                       | NvSciColorStd_REC709_SR   |
 * |                                       | NvSciColorStd_REC709_ER   |
 * |                                       | NvSciColorStd_REC2020_SR  |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Y12                        | NvSciColorStd_REC601_SR   |
 * |                                       | NvSciColorStd_REC601_ER   |
 * |                                       | NvSciColorStd_REC709_SR   |
 * |                                       | NvSciColorStd_REC709_ER   |
 * |                                       | NvSciColorStd_REC2020_SR  |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Y16                        | NvSciColorStd_REC601_SR   |
 * |                                       | NvSciColorStd_REC2020_ER  |
 * |                                       | NvSciColorStd_REC709_ER   |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_U8                         | NvSciColorStd_REC601_SR   |
 * |                                       | NvSciColorStd_REC601_ER   |
 * |                                       | NvSciColorStd_REC709_SR   |
 * |                                       | NvSciColorStd_REC709_ER   |
 * |                                       | NvSciColorStd_REC2020_SR  |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_V8                         | NvSciColorStd_REC601_SR   |
 * |                                       | NvSciColorStd_REC601_ER   |
 * |                                       | NvSciColorStd_REC709_SR   |
 * |                                       | NvSciColorStd_REC709_ER   |
 * |                                       | NvSciColorStd_REC2020_SR  |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_U10                        | NvSciColorStd_REC601_SR   |
 * |                                       | NvSciColorStd_REC601_ER   |
 * |                                       | NvSciColorStd_REC709_SR   |
 * |                                       | NvSciColorStd_REC709_ER   |
 * |                                       | NvSciColorStd_REC2020_SR  |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_V10                        | NvSciColorStd_REC601_SR   |
 * |                                       | NvSciColorStd_REC601_ER   |
 * |                                       | NvSciColorStd_REC709_SR   |
 * |                                       | NvSciColorStd_REC709_ER   |
 * |                                       | NvSciColorStd_REC2020_SR  |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_U12                        | NvSciColorStd_REC601_SR   |
 * |                                       | NvSciColorStd_REC601_ER   |
 * |                                       | NvSciColorStd_REC709_SR   |
 * |                                       | NvSciColorStd_REC709_ER   |
 * |                                       | NvSciColorStd_REC2020_SR  |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_V12                        | NvSciColorStd_REC601_SR   |
 * |                                       | NvSciColorStd_REC601_ER   |
 * |                                       | NvSciColorStd_REC709_SR   |
 * |                                       | NvSciColorStd_REC709_ER   |
 * |                                       | NvSciColorStd_REC2020_SR  |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_U16                        | NvSciColorStd_REC601_SR   |
 * |                                       | NvSciColorStd_REC2020_ER  |
 * |                                       | NvSciColorStd_REC709_ER   |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_V16                        | NvSciColorStd_REC601_SR   |
 * |                                       | NvSciColorStd_REC2020_ER  |
 * |                                       | NvSciColorStd_REC709_ER   |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_A8Y8U8V8                   | NvSciColorStd_REC601_SR   |
 * |                                       | NvSciColorStd_REC601_ER   |
 * |                                       | NvSciColorStd_REC709_SR   |
 * |                                       | NvSciColorStd_REC709_ER   |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Y8U8Y8V8                   | NvSciColorStd_REC601_SR   |
 * |                                       | NvSciColorStd_REC601_ER   |
 * |                                       | NvSciColorStd_REC709_SR   |
 * |                                       | NvSciColorStd_REC709_ER   |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Y8V8Y8U8                   | NvSciColorStd_REC601_SR   |
 * |                                       | NvSciColorStd_REC601_ER   |
 * |                                       | NvSciColorStd_REC709_SR   |
 * |                                       | NvSciColorStd_REC709_ER   |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_U8Y8V8Y8                   | NvSciColorStd_REC601_SR   |
 * |                                       | NvSciColorStd_REC601_ER   |
 * |                                       | NvSciColorStd_REC709_SR   |
 * |                                       | NvSciColorStd_REC709_ER   |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_V8Y8U8Y8                   | NvSciColorStd_REC601_SR   |
 * |                                       | NvSciColorStd_REC601_ER   |
 * |                                       | NvSciColorStd_REC709_SR   |
 * |                                       | NvSciColorStd_REC709_ER   |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_A16Y16U16V16               | NvSciColorStd_REC601_SR   |
 * |                                       | NvSciColorStd_REC601_ER   |
 * |                                       | NvSciColorStd_REC709_SR   |
 * |                                       | NvSciColorStd_REC709_ER   |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_A8                         | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Signed_A8                  | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_B8G8R8A8                   | NvSciColorStd_SRGB        |
 * |                                       | NvSciColorStd_REC2020_RGB |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_A8R8G8B8                   | NvSciColorStd_SRGB        |
 * |                                       | NvSciColorStd_REC2020_RGB |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_A8B8G8R8                   | NvSciColorStd_SRGB        |
 * |                                       | NvSciColorStd_REC2020_RGB |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_A2R10G10B10                | NvSciColorStd_SRGB        |
 * |                                       | NvSciColorStd_REC2020_RGB |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_A16                        | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Signed_A16                 | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Signed_R16G16              | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_A16B16G16R16               | NvSciColorStd_SRGB        |
 * |                                       | NvSciColorStd_REC2020_RGB |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Signed_A16B16G16R16        | NvSciColorStd_SRGB        |
 * |                                       | NvSciColorStd_REC2020_RGB |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Float_A16B16G16R16         | NvSciColorStd_SRGB        |
 * |                                       | NvSciColorStd_SENSOR_RGBA |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_A32                        | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Signed_A32                 | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Signed_X12Bayer20GBRG      | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Float_A16                  | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X6Bayer10BGGI_RGGI         | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X6Bayer10GBIG_GRIG         | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X6Bayer10GIBG_GIRG        | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X6Bayer10IGGB_IGGR         | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X6Bayer10RGGI_BGGI         | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X6Bayer10GRIG_GBIG         | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X6Bayer10GIRG_GIBG         | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X6Bayer10IGGR_IGGB         | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X4Bayer12BGGI_RGGI         | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X4Bayer12GBIG_GRIG         | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X4Bayer12GIBG_GIRG         | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X4Bayer12IGGB_IGGR         | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X4Bayer12RGGI_BGGI         | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X4Bayer12GRIG_GBIG         | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X4Bayer12GIRG_GIBG         | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X4Bayer12IGGR_IGGB         | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X2Bayer14BGGI_RGGI         | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X2Bayer14GBIG_GRIG         | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X2Bayer14GIBG_GIRG         | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X2Bayer14IGGB_IGGR         | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X2Bayer14RGGI_BGGI         | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X2Bayer14GRIG_GBIG         | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X2Bayer14GIRG_GIBG         | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X2Bayer14IGGR_IGGB         | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Bayer16BGGI_RGGI           | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Bayer16GBIG_GRIG           | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Bayer16GIBG_GIRG           | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Bayer16IGGB_IGGR           | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Bayer16RGGI_BGGI           | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Bayer16GRIG_GBIG           | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Bayer16GIRG_GIBG           | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Bayer16IGGR_IGGB           | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_R8                         | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_G8                         | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_B8                         | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Y10_U10__Y10_V10           | NvSciColorStd_REC601_SR   |
 * |                                       | NvSciColorStd_REC601_ER   |
 * |                                       | NvSciColorStd_REC709_SR   |
 * |                                       | NvSciColorStd_REC709_ER   |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Y10_V10__Y10_U10           | NvSciColorStd_REC601_SR   |
 * |                                       | NvSciColorStd_REC601_ER   |
 * |                                       | NvSciColorStd_REC709_SR   |
 * |                                       | NvSciColorStd_REC709_ER   |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_U10_Y10__V10_Y10           | NvSciColorStd_REC601_SR   |
 * |                                       | NvSciColorStd_REC601_ER   |
 * |                                       | NvSciColorStd_REC709_SR   |
 * |                                       | NvSciColorStd_REC709_ER   |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_V10_Y10__U10_Y10           | NvSciColorStd_REC601_SR   |
 * |                                       | NvSciColorStd_REC601_ER   |
 * |                                       | NvSciColorStd_REC709_SR   |
 * |                                       | NvSciColorStd_REC709_ER   |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_R8_G8_B8                   | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_B8_G8_R8                   | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Y10t                       | NvSciColorStd_REC601_SR   |
 * |                                       | NvSciColorStd_REC601_ER   |
 * |                                       | NvSciColorStd_REC709_SR   |
 * |                                       | NvSciColorStd_REC709_ER   |
 * |                                       | NvSciColorStd_REC2020_SR  |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_U10t                       | NvSciColorStd_REC601_SR   |
 * |                                       | NvSciColorStd_REC601_ER   |
 * |                                       | NvSciColorStd_REC709_SR   |
 * |                                       | NvSciColorStd_REC709_ER   |
 * |                                       | NvSciColorStd_REC2020_SR  |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_V10t                       | NvSciColorStd_REC601_SR   |
 * |                                       | NvSciColorStd_REC601_ER   |
 * |                                       | NvSciColorStd_REC709_SR   |
 * |                                       | NvSciColorStd_REC709_ER   |
 * |                                       | NvSciColorStd_REC2020_SR  |
 ...
 * |                                       | NvSciColorStd_REC2020_SR  |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_U10V10t                    | NvSciColorStd_REC601_SR   |
 * |                                       | NvSciColorStd_REC601_ER   |
 * |                                       | NvSciColorStd_REC709_SR   |
 * |                                       | NvSciColorStd_REC709_ER   |
 * |                                       | NvSciColorStd_REC2020_SR  |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_V10U10t                    | NvSciColorStd_REC601_SR   |
 * |                                       | NvSciColorStd_REC601_ER   |
 * |                                       | NvSciColorStd_REC709_SR   |
 * |                                       | NvSciColorStd_REC709_ER   |
 * |                                       | NvSciColorStd_REC2020_SR  |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X2Rc10Rb10Ra10_Bayer10RGGB | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X2Rc10Rb10Ra10_Bayer10BGGR | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X2Rc10Rb10Ra10_Bayer10GRBG | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X2Rc10Rb10Ra10_Bayer10GBRG | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X2Rc10Rb10Ra10_Bayer10CRBC | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X2Rc10Rb10Ra10_Bayer10CBRC | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X2Rc10Rb10Ra10_Bayer10RCCB | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_X2Rc10Rb10Ra10_Bayer10BCCR | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Bayer12RGGB                | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Bayer12BGGR                | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Bayer12GRBG                | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Bayer12GBRG                | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Bayer12CRBC                | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Bayer12CBRC                | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Bayer12RCCB                | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * | NvSciColor_Bayer12BCCR                | NvSciColorStd_SRGB        |
 * +---------------------------------------+---------------------------+
 * </pre>
 *
 * @implements{18840084}
...
typedef enum {
    ...
    NvSciSurfBPC_8,
    /** 10 bits per component (padded into a 16-bit word) */
    NvSciSurfBPC_10,
    /** 12 bits per component (padded into a 16-bit word) */
    NvSciSurfBPC_12,
    /** 14 bits per component (padded into a 16-bit word)
     *
    ...
    NvSciSurfBPC_16,
    /** 10 bits per component (tightly packed) */
    NvSciSurfBPC_10t,
...
typedef enum {
    ...
    NvSciBufHwEngName_MTK_ISP   = 126,
    // Shared Video Pixel Processing
    NvSciBufHwEngName_MTK_SVPP  = 127,
    // Warp Engine
    NvSciBufHwEngName_MTK_WPE   = 128,
    // NVJPG
    NvSciBufHwEngName_MTK_NVJPG = 129,

    // Camera Over Ethernet
    NvSciBufHwEngName_COE       = 130,

    // Placeholder
    NvSciBufHwEngName_Num       = 131
} NvSciBufHwEngName;
...
typedef enum {
    ...
    /** Late attaching peer(s) runs on DRIVEOS CX1 platform **/
    NvSciBufPlatformName_CX1,
 ...
 *
 * @pre @id{NvSciBufAttrListCreate_Precond_001} @asil{QM}
 ...
 *
 * @pre @id{NvSciBufAttrListFree_Precond_001} @asil{QM}
...
/**
 * @brief Sets the values for NvSciBufAttrKey(s) in the slot 0 of input @ref NvSciBufAttrList.
 ...
 *
 * @pre @id{NvSciBufAttrListSetAttrs_Precond_001} @asil{QM}
 *  Valid unreconciled NvSciBufAttrList is obtained.
 * @post @id{NvSciBufAttrListSetAttrs_Postcond_001} @asil{QM}
 *  pairArray values are copied to attrList. Set attributes are made
 * non-writeable in NvSciBufAttrList.
 *
 * @usage
 * - Allowed context for the API call
 *   - Interrupt handler: No
 ...
 *
 * @pre @id{NvSciBufAttrListGetSlotCount_PreCond_001} @asil{B}
 * Valid NvSciBufAttrList is obtained.
 *
 * @usage
 * - Allowed context for the API call
 ...
 * from slot 0 of input @ref NvSciBufAttrList.
 ...
 *      - @a attrList is invalid
 *
 * @pre @id{NvSciBufAttrListGetAttrs_PreCond_001} @asil{B}
 *  Valid NvSciBufAttrList is obtained.
 * @post @id{NvSciBufAttrListGetAttrs_PostCond_001}
 *  If an attribute was never set, the corresponding value will be set to NULL and length to 0.
 * @arr @id{NvSciBufAttrListGetAttrs_RES_001} @asil{D}
 *  The return values, stored in NvSciBufAttrKeyValuePair, consist of
 * const void* pointers to the attribute values from NvSciBufAttrList.
 ...
 *      - @a attrList is invalid
 *
 * @pre @id{NvSciBufAttrListSlotGetAttrs_PreCond_001} @asil{B}
 *  Valid NvSciBufAttrList is obtained.
 * @post @id{NvSciBufAttrListSlotGetAttrs_PostCond_001}
 *  If an attribute was never set, the corresponding value will be set to NULL
 * and length to 0.
 * @arr @id{NvSciBufAttrListSlotGetAttrs_RES_001} @asil{D}
 *  The returned pairArray consist of const void* pointers to the actual attribute
 * values from NvSciBufAttrList. The application must not overwrite this data.
 *
 * @usage
 * - Allowed context for the API call
 *   - Interrupt handler: No
 ...
 * @endif
 */
/// @cond (SWDOCS_NVSCIBUF_NOTSUPPORT)
 ...
 *
 * @pre @id{NvSciBufAttrListReconcile_PreCond_001} @asil{QM}
 *  Valid unreconciled NvSciBufAttrList(s) are obtained.
 * @post @id{NvSciBufAttrListReconcile_PostCond_001}
 *  newReconciledList is valid only if the reconciliation is successful.
 *
 * @usage
 * - Allowed context for the API call
 *   - Interrupt handler: No
 ...
 *
 * @pre @id{NvSciBufAttrListClone_PreCond_001} @asil{QM}
 *  Valid NvSciBufAttrList is obtained.
 * @post @id{NvSciBufAttrListClone_PostCond_001}
 *  If the original NvSciBufAttrList is unreconciled, then modification will be
 * allowed on the cloned NvSciBufAttrList using set attributes APIs even if the
 * attributes had been set in the original NvSciBufAttrList, but the calls to
 * set attributes in either NvSciBufAttrList will not affect the other.
 *
 * @usage
 * - Allowed context for the API call
 *   - Interrupt handler: No
 ...
 *
 * @pre @id{NvSciBufAttrListAppendUnreconciled_PreCond_001} @asil{QM}
 *  Valid unreconciled NvSciBufAttrList(s) are obtained.
 * @post @id{NvSciBufAttrListAppendUnreconciled_PostCond_001}
 *  Appended NvSciBufAttrList is read-only and the attribute values in the
 ...
 *
 * @pre @id{NvSciBufAttrListIsReconciled_PreCond_001} @asil{QM}
 *  Valid NvSciBufAttrList is obtained.
 *
 * @usage
 * - Allowed context for the API call
 ...
 *
 * @pre @id{NvSciBufAttrListValidateReconciled_PreCond_001} @asil{QM}
 *  Valid reconciled NvSciBufAttrList is obtained.
 * @pre @id{NvSciBufAttrListValidateReconciled_PreCond_002} @asil{QM}
 *  Valid unreconciled NvSciBufAttrList(s) are obtained.
 *
 * @usage
 * - Allowed context for the API call
 ...
 *
 * @pre @id{NvSciBufObjDup_PreCond_001} @asil{QM}
 *  Valid NvSciBufObj is obtained.
 * @post @id{NvSciBufObjDup_PostCond_001}
 *  The duplicated NvSciBufObj is not a completely new NvSciBufObj. Therefore,
 * read write operations from one affects the state the other, because it is the same underlying memory.
 * @arr @id{NvSciBufDupObjDup_REC_001} @asil{QM}
 *  The resulting NvSciBufDupObj must be freed separately by the user.
 *
 * @usage
 * - Allowed context for the API call
 *   - Interrupt handler: No
 ...
 *
 * @pre @id{NvSci_BufAttrListReconcileAndObjAlloc_PreCond_001} @asil{QM}
 *  Valid unreconciled NvSciBufAttrList(s) are obtained.
 * @arr @id{NvSciBufAttrListReconcileAndObjAlloc_RES_001} @asil{QM}
 *  The allocator process will have read write permission to the newly Allocated NvSciBufObj.
 *
 * @usage
 * - Allowed context for the API call
 *   - Interrupt handler: No
...
/**
 * @brief Removes reference to the Memory object by freeing the NvSciBufObj.
 *
 ...
 *   - @a bufObj is invalid.
 *
 * @pre @id{NvSciBufObjFree_PreCond_001} @asil{QM}
 *  Valid NvSciBufObj is obtained.
 *
 * @usage
 * - Allowed context for the API call
 ...
 * - Panics if @a bufObj is invalid.
 *
 * @pre @id{NvSciBufObjGetAttrList_PreCond_001} @asil{B}
 *  Valid NvSciBufObj is obtained.
 * @post @id{NvSciBufObjGetAttrList_PostCond_001}
 *  The retrieved NvSciBufAttrList from an NvSciBufObj is read-only
 * and the attribute values in the list cannot be modified using set attribute APIs.
 * @arr @id{NvSciBufObjGetAttrList_RES_001} @asil{D}
 *  The retrieved NvSciBufAttrList must not be freed with NvSciBufAttrListFree().
 *
 * @usage
 * - Allowed context for the API call
 *   - Interrupt handler: No
...
/**
 * @brief Gets the CPU virtual address (VA) of the buffer
 * referenced by the NvSciBufObj. Writes to the buffer will result in undefined
 * behavior when the NvSciBufObj only has NvSciBufAccessPerm_ReadOnly permissions.
 *
 ...
 *      - @a bufObj is NULL.
 *      - @a ptr is NULL.
 * - NvSciError_BadParameter NvSciBufObj did not request
 *   for CPU access by setting NvSciBufGeneralAttrKey_NeedCpuAccess
 *   to true.
 * - Panics if @a bufObj is invalid.
 *
 * @pre @id{NvSciBufObjGetCpuPtr_PreCond_001} @asil{B}
 *  Valid NvSciBufObj is obtained.
 *
 * @usage
 * - Allowed context for the API call
 ...
 * - Panics if @a bufObj is invalid.
 *
 * @pre @id{NvSciBufObjGetConstCpuPtr_PreCond_001} @asil{B}
 *  Valid NvSciBufObj is obtained.
 *
 * @usage
 * - Allowed context for the API call
 ...
 * - Panics if @a bufObj is invalid.
 *
 * @pre @id{NvSciBufObjFlushCpuCacheRange_PreCond_001} @asil{B}
 *  Valid NvSciBufObj is obtained.
 *
 * @usage
 * - Allowed context for the API call
 ...
 *
 * @pre @id{NvSciBufObjAlloc_PreCond_001} @asil{QM}
 *  Valid reconciled NvSciBufAttrList is obtained.
 * @arr @id{NvSciBufObjAlloc_RES_001} @asil{QM}
 *  The allocator process will have read write permission to the newly Allocated NvSciBufObj.
 * @arr @id{NvSciBufObjAlloc_REC_002} @asil{QM}
 *  This function does not take ownership of the reconciled NvSciBufAttrList.
 * The caller remains responsible for freeing the reconciled NvSciBufAttrList.
 * The caller may free the reconciled NvSciBufAttrList any time after this
 * function is called.
 * @arr @id{NvSciBufObjAlloc_RES_003} @asil{D}
 *  It is not guaranteed that the input reconciled NvSciBufAttrList in this
 *  API is the same NvSciBufAttrList that is associated with the allocated
 *  NvSciBufObj. However, the attributes describing CPU accessibility and
 *  buffer size are guaranteed to be the same in both NvSciBufAttrLists.
 *
 *  However, in general, it is recommended that in order to query the
 *  value(s) of attribute(s) from the reconciled NvSciBufAttrList associated
 *  with an NvSciBufObj after allocation, they must first obtain the reconciled
 *  NvSciBufAttrList from the NvSciBufObj using NvSciBufObjGetAttrList()
 *  rather than using a saved NvSciBufAttrList.
 *
 * @usage
 * - Allowed context for the API call
 *   - Interrupt handler: No
 ...
 *
 * If the permissions are the same as permissions of the original object,
 * then the new object counts as a duplicate of the original object.
 ...
 *
 * @pre @id{NvSciBufObjDupWithReducePerm_PreCond_001} @asil{QM}
 *  Valid NvSciBufObj is obtained.
 * @post @id{NvSciBufObjDupWithReducePerm_PostCond_001}
 *  The duplicated NvSciBufObj is not a completely new NvSciBufObj. Therefore,
 * read write operations from one affects the state the other, because it is the same underlying memory.
 * @arr @id{NvSciBufObjDupWithReducePerm_REC_001} @asil{QM}
 *  The resulting NvScBufObj must be freed separately by the user.
 *
 * @usage
 * - Allowed context for the API call
 *   - Interrupt handler: No
 ...
 * must be non-NULL.
 * @param[in] dstPtrSizes an array of plane's sizes. Each size in the
 ...
 *
 * @pre @id{NvSciBufObjGetPixels_PreCond_001} @asil{QM}
 *  Valid NvSciBufObj is obtained.
 * @arr @id{NvSciBufObjGetPixels_RES_001} @asil{D}
 *  User must ensure to read the NvSciBufGeneralAttrKey_CpuNeedSwCacheCoherency attribute value from the reconciled
 * NvSciBufAttrList associated with the NvSciBufObj and perform CPU cache flush
 * operation using NvSciBufObjFlushCpuCacheRange() API before calling NvSciBufObjGetPixels()
 * if the attribute value is set to TRUE.
 * @arr @id{NvSciBufObjGetPixels_REC_002} @asil{B}
 *  The usage of this API must be restricted for use in debug software only and must not be used in production software.
 *
 * @usage
 * - Allowed context for the API call
 *   - Interrupt handler: No
 ...
 * be non-NULL.
 * @param[in] srcPtrSizes an array of plane's sizes. Each size in the
 ...
 *  - @a NvSciBufAttrList associated with the @a bufObj is invalid.
 *
 * @pre @id{NvSciBufObjSetPixels_PreCond_001} @asil{QM}
 *  Valid NvSciBufObj is obtained.
 * @arr @id{NvSciBufObjSetPixels_RES_001} @asil{D}
 *  User must ensure to read the NvSciBufGeneralAttrKey_CpuNeedSwCacheCoherency attribute value from the reconciled
 * NvSciBufAttrList associated with the NvSciBufObj and perform CPU cache flush
 * operation using NvSciBufObjFlushCpuCacheRange() API after calling NvSciBufObjPutPixels()
 * if the attribute value is set to TRUE.
 * @arr @id{NvSciBufObjSetPixels_REC_002} @asil{B}
 *  The usage of this API must be restricted for use in debug software only and must not be used in production software.
 *
 * @usage
 * - Allowed context for the API call
 ...
 *
 * @pre @id{NvSciBufIpcExportAttrListAndObj_PreCond_001} @asil{QM}
 *  Valid NvSciBufObj obtained.
 * @pre @id{NvSciBufIpcExportAttrListAndObj_PreCond_002} @asil{QM}
 ...
 *    - @a module is invalid
 *
 * @pre @id{NvSciBufIpcImportAttrListAndObj_PreCond_001} @asil{QM}
 *  Valid NvSciBufModule is obtained.
 * @pre @id{NvSciBufIpcImportAttrListAndObj_PreCond_002} @asil{QM}
 *  Valid unreconciled NvSciBufAttrList(s) are obtained.
 * @pre @id{NvSciBufIpcImportAttrListAndObj_PreCond_003} @asil{QM}
 *  Valid NvSciIpcEndpoint is obtained.
 *
 * @usage
 * - Allowed context for the API call
 ...
 *
 * @pre @id{NvSciBufAttrListAndObjFreeDesc_PreCond_001} @asil{QM}
 *  Valid descriptor used for exporting both NvSciBufAttrList and NvSciBufObj together is obtained.
 *
 * @usage
 * - Allowed context for the API call
 *   - Interrupt handler: No
 ...
 *            Valid value: The NvSciIpcEndpoint should be obtained from a
 *            successful call to NvSciIpcOpenEndpoint() and has not yet been
 *            freed using NvSciIpcCloseEndpointSafe().
 ...
 * - Panic if @a bufObj is invalid
 *
 * @pre @id{NvSciBufObjIpcExport_PreCond_001} @asil{QM}
 *  Valid NvSciBufObj is obtained.
 * @pre @id{NvSciBufObjIpcExport_PreCond_002} @asil{QM}
 *  Valid NvSciIpcEndpoint is obtained.
 *
 * @usage
 * - Allowed context for the API call
 ...
 *
 * @pre @id{NvSciBufObjIpcImport_PreCond_001} @asil{QM}
 *  Valid NvSciBufObjIpcExportDescriptor is obtained.
 * @pre @id{NvSciBufObjIpcImport_PreCond_002} @asil{QM}
 *  Valid reconciled NvSciBufAttrList is obtained.
 * @pre @id{NvSciBufObjIpcImport_PreCond_003} @asil{QM}
 *  Valid NvSciIpcEndpoint is obtained.
 * @arr @id{NvSciBufObjIpcImport_REC_001} @asil{QM}
 *  NvSciBufObjIpcImport() does not take ownership of input NvSciBufAttrList.
 * The caller remains responsible for freeing input NvSciBufAttrList. The caller
 * may free the input NvSciBufAttrList any time after this function is called.
 * @arr @id{NvSciBufObjIpcImport_RES_002} @asil{B}
 *  Input NvSciBufObjIpcExportDescriptor can be used to import NvSciBufObj only once
 * i.e the same desc cannot be used to import NvSciBufObj multiple times.
 *
 * @usage
 * - Allowed context for the API call
 *   - Interrupt handler: No
 ...
 *
 * @pre @id{NvSciBufObjAttachPeer_PreCond_001} @asil{QM}
 *  Valid NvSciBufObj is allocated.
 * @pre @id{NvSciBufObjAttachPeer_PreCond_002} @asil{QM}
 *  Valid unreconciled NvSciBufAttrList(s) inside @a inputArray.
 * @post @id{NvSciBufObjAttachPeer_PostCond_001}
 *  NvSciBufObj and reconciled NvSciBufAttrList associated with @a bufObj is exportable
 * to remote peers NvSciIpcEndpoints whose unreconciled NvSciBufAttrList was provided
 ...
 *   is invalid.
 *
 * @pre @id{NvSciBufAttrListIpcExportUnreconciled_PreCond_001} @asil{QM}
 *  Valid unreconciled NvSciBufAttrList(s) are obtained.
 * @pre @id{NvSciBufAttrListIpcExportUnreconciled_PreCond_002} @asil{QM}
 *  Valid NvSciIpcEndpoint is obtained.
 * @post @id{NvSciBufAttrListIpcExportUnreconciled_PostCond_001}
 *  When exporting an array containing multiple unreconciled
 * NvSciBufAttrLists, the importing endpoint still imports just one unreconciled
 * NvSciBufAttrList. This unreconciled NvSciBufAttrList is referred to as a
 * multi-slot NvSciBufAttrList. It logically represents an array of
 * NvSciBufAttrLists.
 *
 * @usage
 * - Allowed context for the API call
 ...
 * - Panic if @a reconciledAttrList is invalid.
 *
 * @pre @id{NvSciBufAttrListIpcExportReconciled_PreCond_001} @asil{QM}
 *  Valid reconciled NvSciBufAttrList is obtained.
 * @pre @id{NvSciBufAttrListIpcExportReconciled_PreCond_002} @asil{QM}
 *  Valid NvSciIpcEndpoint is obtained.
 *
 * @usage
 * - Allowed context for the API call
 ...
 *
 * @pre @id{NvSciBufAttrListIpcImportUnreconciled_PreCond_001} @asil{QM}
 *  Valid NvSciBufModule is obtained.
 * @pre @id{NvSciBufAttrListIpcImportUnreconciled_PreCond_002} @asil{QM}
 *  Valid NvSciIpcEndpoint is obtained.
 * @post @id{NvSciBufAttrListAppendUnreconciled_PostCond_001}
 *  Imported NvSciBufAttrList is no longer writable.
 *
 * @usage
 * - Allowed context for the API call
 *   - Interrupt handler: No
 ...
 *      - @a module is invalid
 *
 * @pre @id{NvSciBufAttrListIpcImportReconciled_PreCond_001} @asil{QM}
 *  Valid NvSciBufModule is obtained.
 * @pre @id{NvSciBufAttrListIpcImportReconciled_PreCond_002} @asil{QM}
 *  Valid unreconciled NvSciBufAttrList(s) are obtained.
 * @pre @id{NvSciBufAttrListIpcImportReconciled_PreCond_003} @asil{QM}
 *  Valid NvSciIpcEndpoint is obtained.
 * @arr @id{NvSciBufAttrListIpcImportReconciled_REC_001} @asil{QM}
 *  It is recommended that the importing process validates the reconciled
 * NvSciBufAttrList against the input unreconciled NvSciBufAttrList(s),
 * so that the importing process can be sure that an NvSciBufObj will
 ...
 *     - @a descBuf is invalid
 *
 * @pre @id{NvSciBufAttrListFreeDesc_PreCond_001} @asil{QM}
 ...
 *   - @a module is invalid
 *
 * @pre @id{NvSciBufModuleClose_PreCond_001} @asil{QM}
 *  Valid NvSciBufModule is obtained.
 *
 * @usage
 * - Allowed context for the API call
 ...
 * Valid value: @a majorVer is NvSciBufMajorVersion.
 * @param[in] minorVer build minor version.
 * Valid value: [0 - <= NvSciBufMajorVersion].
 ...
 *
 * @pre @id{NvSciBufObjValidate_PreCond_001} @asil{B}
 *  Valid NvSciBufObj is obtained.
 * @post @id{NvSciBufObjValidate_PostCond_001}
 *  NvSciBufObj is marked as validated if successful.
 *
 * \endif
 *
 * @usage
 ...
 * - panics if @a reconciledAttrList is invalid.
 *
 * @pre @id{NvSciBufAttrListValidateReconciledAgainstAttrs_PreCond_001} @asil{B}
 *  Valid reconciled NvSciBufAttrList is obtained.
 *
 * @usage
 * - Allowed context for the API call
 ...
 * - panics if @a bufObj is invalid.
 *
 * @pre @id{NvSciBufObjGetMaxPerm_PreCond_001} @asil{B}
 *  Valid NvSciBufObj is obtained.
 * @pre @id{NvSciBufObjGetMaxPerm_PreCond_002} @asil{QM}
 *  Valid NvSciIpcEndpoint is obtained.
 *
 * @usage
 * - Allowed context for the API call
...
/**
 * @brief Generate a new dma-buf file descriptor corresponding to the provided
 * NvSciBufObj.
 *
 * <b> This API is specific to Linux OS. </b>
 *
 * @param[in] bufObj NvSciBufObj from which new file descriptor is generated
 * @param[out] fd dma-buf File Descriptor corresponding to the underlying
 * memory of the NvSciBufObj. The caller owns this returned file descriptor and
 ...
 *      - @a fd is NULL.
 *      - @a fd is NULL.
 *      - @a bufObj did not request for CPU access by setting
 *        NvSciBufGeneralAttrKey_NeedCpuAccess to true
 * - NvSciError_ResourceError if system lacks resource other than memory.
 * - panics if @a bufObj is invalid.
 * \else
 * @return NvSciError, the completion code of the operation:
 ...
 * - NvSciError_InvalidState If the Init Mode API is called in Runtime Mode.
 * \endif
 * @pre @id{NvSciBufObjGetDmaBufFd_PreCond_001} @asil{B}
 *  Valid NvSciBufObj is obtained.
 * @arr @id{NvSciBufObjGetDmaBufFd_RES_001} @asil{D}
 *  The returned file descriptor shall not be shared with another process
 * @arr @id{NvSciBufObjGetDmaBufFd_RES_002} @asil{D}
 *  The returned file descriptor shall not be duplicated
 *
 * @usage
 * - Allowed context for the API call
 *   - Interrupt handler: No
 ...
 *   - De-Init: No
 */
NvSciError NvSciBufObjGetDmaBufFd(
    NvSciBufObj bufObj,
    int* fd);
