/*
 * Header file for NvSciStream Data Types
 *
...
static uint32_t const NvSciStreamMinorVersion = 3U;
...
typedef enum {
    ...
    /*! \brief Specification of combined element and waiter attributes.
     *
     * In the producer and consumer, this group contains the list of
     *   elements supported by the endpoint along with their corresponding
     *   waiter sync attributes. This combines the functionality of
     *   ElementExport and WaiterAttrExport into a single operation.
     *
     * Marking this complete causes both the element attribute list and
     *   waiter sync attribute list to be sent to the pool together.
     *
     * This setup type is mutually exclusive with the separate
     *   NvSciStreamSetup_ElementExport and NvSciStreamSetup_WaiterAttrExport.
     *   If an endpoint uses this combined approach, all endpoints in the
     *   stream must use the same approach.
     *
     * When element and waiter export is complete, the following functions
     *   are no longer available on the block:
     *   - NvSciStreamBlockElementAttrSet()
     *   - NvSciStreamBlockElementWaiterAttrSet()
     *   - NvSciStreamBlockElementAndWaiterAttrSet()
     */
    NvSciStreamSetup_ElementAndWaiterAttrsExport = 0x0003,
...
typedef enum {
    ...
    /*! \brief
     *  Signals that all downstream consumers disconnected.
     *
     *  Received by multicast block only.
     */
    NvSciStreamEventType_ZeroConsumer               = 0x004070,
...
/*! \brief Defines stream configuration keys for the blocks.
 */
typedef enum {

    /*! \brief
     *  Indicates whether the multicast block should propagate the disconnect
     *    message to upstream if the last downstream consumer is disconnected.
     *
     *  Configurate multicast block.
     *
     *  Value: @c uint8_t
     *
     *  - 1 (Default): Disconnect stream when all downstream consumer
     *      disconnected, and propagate the disconnect message upstream to the
     *      producer.
     *
     *  - 0 Keep the stream alive and post the
     *      NvSciStreamEventType_ZeroConsumer event on the multicast block.
     *      Return the packet upstream immediately if the producer presents a
     *      packet when zero consumer connected.
     *      The multicast block can continue accepting new consumer attached.
     *      Stream disconnect will only happen when the multicast block is
     *      deleted or deletion is initiated by the upstream block of the
     *      multicast block.
     */
    NvSciStreamConfigKey_DisconnectWithZeroConsumer = 0x000001,

    /*! \brief End of range for NvSciStreamConfigKey */
    NvSciStreamConfigKey_UpperBound                 = 0x05FFFF

} NvSciStreamConfigKey;


/*! \brief This structure defines a key/value pair used to configure
 *    the NvSciStream block before connection.
 */
typedef struct {
    /*! \brief NvSciStreamConfigKey specifies the stream config type. */
    NvSciStreamConfigKey configKey;

    /*! \brief Pointer to the value corresponding to the config data. */
    const void* configValue;

    /*! \brief Length of the config data in bytes */
    uint32_t configLen;
