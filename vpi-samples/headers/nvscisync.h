 ...
 * \implements{18840180}
 */
static const uint32_t NvSciSyncMinorVersion = 10U;
 ...
 *
 * Each reference has its own permissions which influence which
 * operations are possible to do using this NvSciSyncObj.
 *
...
typedef enum {
     ...
     *
     * This attribute is mandatory if no Nvidia UMD was used for filling
     * attributes.
     *
 ...
 *
 * @note Use this API ONLY if Safety software ASIL level is ASIL-B or lower.
 * For higher level, more robust API NvSciSyncAttrListValidateReconciledAgainstAttrs
 * must be used instead of this API.
 *
 ...
 *  Valid unreconciled NvSciSyncAttrList(s) are obtained.
 * @pre @id{NvSciSyncAttrListIpcExportUnreconciled_PreCond_002} @asil{QM}
 ...
 * @pre @id{NvSciSyncAttrListIpcExportReconciled_PreCond_002} @asil{QM}
 *  Valid NvSciIpcEndpoint is obtained.
 *
 * @usage
 * - Allowed context for the API call
 ...
 *  Valid NvSciSyncModule is obtained.
 * @pre @id{NvSciSyncAttrListIpcImportUnreconciled_PreCond_002} @asil{QM}
 *  Valid NvSciIpcEndpoint is obtained.
 ...
 *  Valid unreconciled NvSciSyncAttrList(s) are obtained.
 * @pre @id{NvSciSyncAttrListIpcImportReconciled_PreCond_003} @asil{QM}
 *  Valid NvSciIpcEndpoint is obtained.
 ...
 * The resulting @a syncObj has permissions equal to the value
 * of NvSciSyncAttrKey_ActualPerm attribute in @a reconciledList.
 *
 ...
 *
 * @a dupObj counts as not validated yet in terms of NvSciSyncObjValidate()
 *
...
/**
 * \brief Creates a new @ref NvSciSyncObj holding a reference to the original
 * resources to which the input @ref NvSciSyncObj holds reference to. The new
 * object has permissions reduced.
 *
 ...
 * Otherwise, it does not. Also, if the permissions were strictly reduced,
 * the new object cannot be exported and cannot have peers attached to it.
 *
 * @a dupObj counts as not validated yet in terms of NvSciSyncObjValidate()
 *
 * \param[in] syncObj NvSciSyncObj to duplicate.
 * \param [in] reducedPerm permissions of the new object
 * Valid value: non-empty NvSciSyncAccessPerm which is <= permissions
 * of the @a syncObj
 * \param[out] dupObj Duplicated NvSciSyncObj.
 * \return ::NvSciError, the completion code of the operation:
 * - ::NvSciError_Success if successful.
 ...
 * - ::NvSciError_BadParameter if any of the following occurs:
 *      - any argument is NULL
 *      - reducedPerm is empty
 *      - reducedPerm > permissions of @a syncObj
 * - ::NvSciError_InsufficientMemory if memory allocation failed.
 * - ::NvSciError_InvalidState if the number of references to the
 *   synchronization object of the input NvSciSyncObj is INT32_MAX and the
 *   newly duplicated NvSciSyncObj tries to take one more reference using this
 ...
 * - Panics if @a syncObj is not valid
 *
 * @pre @id{NvSciSyncObjDupWithReducePerm_PreCond_001} Valid NvSciSyncObj is obtained. @asil{QM}
 * @post @id{NvSciSyncObjDupWithReducePerm_PreCond_001_PostCond_001}
 *  The duplicated NvSciSyncObj is not a completely new NvSciSyncObj. Therefore,
 * signaling and generating NvSciSyncFences from one affects the state of the
 * other, because it is the same underlying NvSciSyncObj.
 * @arr @id{NvSciSyncObjDupWithReducePerm_PreCond_001_PostCond_001_REC_001} @asil{QM}
 *  The resulting NvSciSyncObj must be freed separately by the user.
 *
 * @usage
 * - Allowed context for the API call
 ...
 *   - De-Init: No
 */
NvSciError NvSciSyncObjDupWithReducePerm(
    NvSciSyncObj syncObj,
    NvSciSyncAccessPerm reducedPerm,
    NvSciSyncObj* dupObj);

/**
 * \brief Retrieves permissions from an NvSciSyncObj.
 *
 * These are equal to the value of NvSciSyncAttrKey_ActualPerm
 * in the reconciled list the object was allocated against
 * or reduced permissions the object was duplicated with.
 *
 * Note that the value of NvSciSyncAttrKey_ActualPerm in the reconciled
 * list in this object might differ if the object was duplicated with reduced
 * permissions.
 *
 * \param[in] syncObj NvSciSyncObj to retrieve permissions from.
 * \param [out] permissions permissions of @a syncObj.
 * \return ::NvSciError, the completion code of the operation:
 * - ::NvSciError_Success if successful
 * - ::NvSciError_BadParameter if any argument is NULL
 * - Panics if @a syncObj is not valid
 *
 * @pre @id{NvSciSyncObjGetPerm_PreCond_001} Valid NvSciSyncObj is obtained. @asil{QM}
 *
 * @usage
 * - Allowed context for the API call
 *   - Interrupt handler: No
 ...
 *   - De-Init: No
 */
NvSciError NvSciSyncObjGetPerm(
    NvSciSyncObj syncObj,
 ...
 *      - @a syncObj has permissions reduced from the original NvSciSyncObj
 * - ::NvSciError_InsufficientMemory if memory allocation failed.
 */
#if (NV_L4T == 1)
/**
 ...
 *  Valid NvSciSyncObj is obtained.
 * @pre @id{NvSciSyncObjIpcExport_PreCond_002} @asil{QM}
 *  Valid NvSciIpcEndpoint is obtained.
 *
 * @usage
 * - Allowed context for the API call
 ...
 * The resulting @a syncObj has permissions equal to @a permissions if
 * it is different from NvSciSyncAccessPerm_Auto. Otherwise, it is
 * set to the value granted by the exporter.
 *
 ...
 *  Valid reconciled NvSciSyncAttrList is obtained.
 * @pre @id{NvSciSyncObjIpcImport_PreCond_002} @asil{QM}
 *  Valid NvSciIpcEndpoint is obtained.
 ...
 *  Valid NvSciSyncFence* is obtained.
 * @pre @id{NvSciSyncIpcExportFence_PreCond_002} @asil{QM}
 *  Valid NvSciIpcEndpoint is obtained.
 *
 * @usage
 * - Allowed context for the API call
...
#if (defined(__linux__) || defined (__ANDROID__) || (NV_L4T == 1)) && (NV_IS_SAFETY == 0)
/**
 * \brief Creates a sync file from NvSciSyncFence
 *
 * \param[in] syncFence valid NvSciSyncFence
 * \param[out] fenceFd The file descriptor of the created sync file
 *
 * \return NvSciError
 * - NvSciError_Success if successful.
 * - NvSciError_BadParameter if any of the following occurs:
 *         - @a syncFence or @a fenceFd is NULL
 * - NvSciError_NotSupported if @a NvSciSyncObj associated with syncFence is not
 *   backed by NvSciSyncAttrValPrimitiveType_Syncpoint
 * - NvSciError_ResourceError if failed to create fenceFd.
 * - NvSciError_ClearedFence if syncFence is a valid cleared NvSciSyncFence
 * - Panics if NvSciSyncObj associated with syncFence is invalid
 *
 * @pre @id{NvSciSyncFencePutToFile_PreCond_001} @asil{QM}
 *  Valid NvSciSyncFence* is obtained.
 * @arr @id{NvSciSyncFencePutToFile_REC_001} @asil{QM}
 *  If created successfully, the fenceFd has to be explicitly freed by the caller using close().
 *
 * @usage
 * - Allowed context for the API call
 *   - Interrupt handler: No
 ...
 *   - De-Init: No
 */
NvSciError NvSciSyncFencePutToFile(
    const NvSciSyncFence* syncFence,
    int32_t* fenceFd);

/**
 * \brief Populates the input NvSciSyncFence based on the input sync file.
 *
 * \param[in] syncObj valid NvSciSyncObj
 * \param[in] fenceFd The file descriptor of the sync file
 * \param[in,out] syncFence NvSciSyncFence to populate
 *
 * \return NvSciError
 * - NvSciError_Success if successful.
 * - NvSciError_BadParameter if any of the following occurs:
 *         - @a syncFence or @a syncObj is NULL
 * - NvSciError_NotSupported if @a NvSciSyncObj is not backed by
 *   NvSciSyncAttrValPrimitiveType_Syncpoint
 * - NvSciError_InvalidState if no more references can be taken on
 *   the syncObj
 * - NvSciError_ResourceError in following cases:
 *         - @a fenceFd is invalid or contains multiple fences
 *         - any other translational call failure
 * - Panics if syncObj or NvSciSyncObj initially associated with syncFence
 *   is invalid
 *
 * @pre @id{NvSciSyncFenceUpdateFromFile_PreCond_001} @asil{QM}
 *  Valid NvSciSyncObj is obtained.
 * @pre @id{NvSciSyncFenceUpdateFromFile_PreCond_002} @asil{QM}
 *  Valid NvSciSyncFence* is obtained.
 * @pre @id{NvSciSyncFenceUpdateFromFile_PreCond_003} @asil{QM}
 *  Valid sync file descriptor is obtained.
 * @post @id{NvSciSyncFenceUpdateFromFile_PostCond_001}
 *  This new NvSciSyncFence is associated with the input NvSciSyncObj. The input
 * NvSciSyncFence is cleared before being populated with the new data.
 * @arr @id{NvSciSyncFenceUpdateFromFile_REC_001} @asil{QM}
 *  The task status slot associated with this fence will be 0. It is recommended
 * to not use this interface when using task status buffer.
 *
 * @usage
 ...
 *   - De-Init: No
 */
NvSciError NvSciSyncFenceUpdateFromFile(
    NvSciSyncObj syncObj,
    int32_t fenceFd,
    NvSciSyncFence* syncFence);
#endif
 ...
 *         - the NvSciSyncObj associated with @a syncFence has no waiting permissions
 * - Panics if NvSciSyncObj associated with @a syncFence is not valid.
 *
 ...
 * NvSciSyncAttrList.
 * The resulting @a syncObj has permissions equal to the value
 * of NvSciSyncAttrKey_ActualPerm attribute in the reconciled attribute list.
 *
 ...
 * @pre @id{NvSciSyncIpcExportAttrListAndObj_PreCond_002} @asil{QM}
 *  Valid NvSciIpcEndpoint is obtained.
 *
 * @usage
 * - Allowed context for the API call
 ...
 *  Valid unreconciled NvSciSyncAttrList(s) are obtained.
 * @pre @id{NvSciSyncIpcImportAttrListAndObj_PreCond_003} @asil{QM}
 *  Valid NvSciIpcEndpoint is obtained.
 *
 * @usage
 * - Allowed context for the API call
 ...
 *
 * This API offers more robust validation than
 * NvSciSyncAttrListValidateReconciled and must be used in Safety software
 * with level higher than ASIL-B.
 *
 * @note It can also be used in Safety software at ASIL-B or lower level.
 * If this API is used, application software does NOT need to use
...
/**
 * \brief Sets the task status for a specific slot in the NvSciSyncObj.
 *
 * This function allows CPU signalers to set task status information before
 * signaling the NvSciSyncObj. The task status will be available to waiters
 * when the fence is signaled.
 *
 * \param[in] syncObj The NvSciSyncObj for which to set task status.
 * \param[in] taskStatusSlot The slot index in the task status buffer.
 * Valid value: [0, number-of-task-status-slots - 1]
 * \param[in] taskStatus Pointer to the task status structure to set.
 *
 * \return NvSciError_Success if the task status was set successfully.
 * \return NvSciError_BadParameter if any of the following occurs:
 *      - @a syncObj is NULL
 *      - @a taskStatusSlot is invalid
 *      - @a taskStatus is NULL
 *      - @a syncObj does not support task status
 *      - NvSciSyncObj does not have CPU signalling permissions
 * - Panics if @a syncObj is invalid.
 *
 * \note This function should be called before signaling the NvSciSyncObj.
 *       The task status will be available to waiters when the fence is signaled.
 *       This function is thread-safe and can be called concurrently from
 *       multiple threads.
 *
 * @pre @id{NvSciSyncObjSetTaskStatus_PreCond_001} @asil{B}
 *  Valid NvSciSyncObj is obtained and supports task status functionality.
 *
 * @usage
 * - Allowed context for the API call
 *   - Interrupt handler: No
 ...
 *   - De-Init: No
 */
NvSciError NvSciSyncObjSetTaskStatus(
    NvSciSyncObj syncObj,
    uint32_t taskStatusSlot,
    const NvSciSyncTaskStatus* taskStatus);
