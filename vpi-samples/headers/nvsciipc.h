 ...
 * @version 1.5
 *
 * @section 1.5 Jul/25/2025
 * - Make internal NvSciIpc APIs public
 *   NvSciIpcOpenCfgBlob()
 *   NvSciIpcGetEndpointAccessInfo()
...
// nvscicom is supported only when QNX non-safety build
#if defined(__QNX__) && (NV_IS_SAFETY == 0)
#define IS_NVSCICOM_ENABLED 1
#endif

#if defined(__QNX__)
/* nvsciipc custom ability can be created or looked up w/ this ID */
#ifndef NVSCIIPC_ABILITY_ID
#define NVSCIIPC_ABILITY_ID "NvSciIpcEndpoint"
#endif /* NVSCIIPC_ABILITY_ID */
#endif /* __QNX__ */
...
#if defined(IS_NVSCICOM_ENABLED)
    #define NVSCIIPC_BACKEND_ITC_NVSCICOM   2U
    #define NVSCIIPC_BACKEND_IPC_NVSCICOM   3U
    #define NVSCIIPC_BACKEND_IVC            4U
    #define NVSCIIPC_BACKEND_C2C            5U
    #define NVSCIIPC_BACKEND_C2C_PCIE       5U
    #define NVSCIIPC_BACKEND_C2C_NPM        6U
    #define NVSCIIPC_BACKEND_MAX            6U     /* (NVSCIIPC_BACKEND_C2C_PCIE + 1U) */
#else
    #define NVSCIIPC_BACKEND_IVC            2U
    #define NVSCIIPC_BACKEND_C2C            3U
    #define NVSCIIPC_BACKEND_C2C_PCIE       3U
    #define NVSCIIPC_BACKEND_C2C_NPM        4U
    #if defined(__linux__)
        /* (NVSCIIPC_BACKEND_C2C_NPM + 1U) */
        #define NVSCIIPC_BACKEND_MAX  5U
    #elif defined(__QNX__)
        /* (NVSCIIPC_BACKEND_C2C_PCIE + 1U) */
        #define NVSCIIPC_BACKEND_MAX  4U
    #endif
#endif /* IS_NVSCICOM_ENABLED */

#define NVSCIIPC_INVALID_IRQ (0xFFFFFFFFU)
#define NVSCIIPC_INVALID_IID (-1)

/**
 * @brief VUID(VM unique ID) of the NvSciIpc endpoint.
 */
typedef uint64_t NvSciIpcEndpointVuid;

typedef struct NvSciIpcEndpointAccessInfo NvSciIpcEndpointAccessInfo;

/**
 * @brief Defines access information about NvSciIpc endpoint
 */

struct NvSciIpcEndpointAccessInfo {
    /*! unique group id of the endpoint */
    uint32_t gid;
    /*! backend type of the endpoint
     * NVSCIIPC_BACKEND_ITC, NVSCIIPC_BACKEND_IPC, NVSCIIPC_BACKEND_IVC, ...
     */
    uint32_t backend;
     ...
     * NVSCIIPC_TRAP_IPA, NVSCIIP_MSI_IPA
     */
    uint32_t notiIpaType;
    /*! User id per endpoint */
    uint32_t uid;
};
...
/**
 * @brief Opens NvSciIpc ConfigBlob.
 *
 * <b>This API is specific to QNX OS.</b>
 * The function opens and read NvSciIpc configuration blob and populates
 * internal DB to search endpoint entry.
 *
 * @return ::NvSciError, the completion code of the operation:
 * - ::NvSciError_Success            Indicates a successful operation.
 * - ::NvSciError_NotPermitted       Indicates opening blob has failed.
 *                                   Indicates incorrect API group usage.
 * - ::NvSciError_NotSupported       Indicates API is not supported in the provided
 *                                   OS environment.
 * - ::NvSciError_InvalidState       Indicates an invalid operation state
 *
 * @usage
 ...
 *   - De-Init: No
 */
NvSciError NvSciIpcOpenCfgBlob(void);
 ...
 * QNX OS abilities to NvSciIpc client program.
 * <b>This API can be used in root user process only due to security issue.</b>
 *
 * @param[in]  endpoint  The name of the NvSciIpc endpoint to search.
 ...
 * - ::NvSciError_NoSuchEntry        Indicates Endpoint name is not found.
 ...
 * - ::NvSciError_InvalidState       Indicates an invalid operation state
 *
 * @pre Invocation of NvSciIpcOpenCfgBlob() must be successful.
 *
 * @usage
 * - Allowed context for the API call
 *   - Interrupt handler: No
 ...
 *   - De-Init: No
 */
NvSciError NvSciIpcGetEndpointAccessInfo(const char *endpoint,
    NvSciIpcEndpointAccessInfo *info);

/**
 * @brief Closes NvSciIpc ConfigBlob.
 *
 * <b>This API is specific to QNX OS.</b>
 * Unmap NvSciIpc configuration blob shared memory and close it.
 *
 * @note This API can be called in Init mode to release resources
 *       in error handling or to test functionality.
 *
 ...
 *   - De-Init: Yes
 */
void NvSciIpcCloseCfgBlob(void);
 ...
 *                            Use handle which NvSciIpcOpenEndpoint* API returns
