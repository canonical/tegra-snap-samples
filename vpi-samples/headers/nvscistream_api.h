/*
 * Header file for NvSciStream APIs
 *
 ...
 * - The input parameter for which the valid value is not stated in the interface
 *   specification and not covered by any of the blanket statements, it is considered
 *   that the entire range of the parameter type is valid.
 *
 ...
 * - If an API function is invoked with parameters such that two or more failure modes
 *   are possible, it is guaranteed that one of those failure modes will occur, but it
 *   is not guaranteed which one would exactly occur.
 *
 ...
 *
 * The NvSciStream library is a layer on top of NvSciBuf, NvSciSync, NvIpc and
 * NvSciEventService libraries that provides utilities for streaming sequences
 * of data packets between multiple application modules to support a wide variety
 * of use cases.
 *
 ...
 *  - ::NvSciError_StreamBadBlock Block referenced by @a upstream or
 *      @a downstream is invalid.
 *  - ::NvSciError_InsufficientResource If any of the following occurs:
 *       - block referenced by @a upstream argument has no available outputs
 *       - block referenced by @a downstream argument has no available inputs
 *  - ::NvSciError_AccessDenied Block referenced by @a upstream argument or
 *      block referenced by @a downstream argument(for example, Pool or Queue)
 *      does not allow explicit connection via NvSciStreamBlockConnect().
 *  - ::NvSciError_InvalidState If any of the following occurs:
 * \if NV_QNX
 *       - API is called in runtime state
 * \endif
 *       - @a upstream argument represents a multicast block which is not ready
 *         for late consumer connections.
 * \if NV_SAFETY
 *  - Panics if locking any internal mutex fails due to system level error.
 * \endif
 ...
 * \if NV_QNX
 *  - ::NvSciError_InvalidState API is called in runtime state.
 * \endif
 *  - ::NvSciError_BadParameter If any of the following occurs:
 *       - the output parameter @a producer is a null pointer
 *       - the @a pool parameter does not reference a pool block
 *  - ::NvSciError_StreamBadBlock If block referenced by @a pool is invalid.
 *  - ::NvSciError_InsufficientResource If the pool block is already
 *      associated with another producer.
 *  - ::NvSciError_StreamInternalError If any of the following occurs:
 *       - block registration fails
 *       - the producer block cannot be initialized properly
 *  - ::NvSciError_InsufficientMemory Memory allocation fails.
 * \if NV_SAFETY
 *  - Panics if locking any internal mutex fails due to system level error.
 * \endif
 * @pre
 ...
 * \endif
 *  - ::NvSciError_BadParameter If any of the following occurs:
 *       - the output parameter @a consumer is a null pointer
 *       - the @a queue parameter does not reference a queue block
 *  - ::NvSciError_StreamBadBlock If block referenced by @a queue is invalid.
 *  - ::NvSciError_InsufficientResource If the queue block is already bound to
 *      another consumer.
 *  - ::NvSciError_StreamInternalError If any of the following occurs:
 *       - block registration fails
 *       - the consumer block cannot be initialized properly
 *  - ::NvSciError_InsufficientMemory Memory allocation fails.
 * \if NV_SAFETY
 *  - Panics if locking any internal mutex fails due to system level error.
 * \endif
 ...
 *       - the output parameter @a consumer is a null pointer
 *       - the @a queue parameter does not reference a queue block
 *  - ::NvSciError_StreamBadBlock If the block referenced by @a queue is
 *      invalid.
 *  - ::NvSciError_InsufficientResource If the queue block is already bound to
 *      another consumer.
 *  - ::NvSciError_StreamInternalError If any of the following occurs:
 ...
 *  - ::NvSciError_StreamInternalError If any of the following occurs:
 *       - block registration fails
 *       - the pool block cannot be initialized properly
 *  - ::NvSciError_InsufficientMemory Memory allocation fails.
 * \if NV_SAFETY
 *  - Panics if locking any internal mutex fails due to system level error.
 * \endif
 ...
 *  - ::NvSciError_StreamInternalError If any of the following occurs:
 *       - block registration fails
 *       - the mailbox queue block cannot be initialized properly
 *  - ::NvSciError_InsufficientMemory Memory allocation fails.
 * \if NV_SAFETY
 *  - Panics if locking any internal mutex fails due to system level error.
 * \endif
 ...
 *  - ::NvSciError_StreamInternalError If any of the following occurs:
 *       - block registration fails
 *       - the FIFO queue block cannot be initialized properly.
 *  - ::NvSciError_InsufficientMemory Memory allocation fails.
 * \if NV_SAFETY
 *  - Panics if locking any internal mutex fails due to system level error.
 * \endif
 ...
 * \endif
 *  - ::NvSciError_BadParameter If any of the following occurs:
 *       - the output parameter @a multicast is a null pointer
 *       - @a outputCount is larger than the number allowed
 *  - ::NvSciError_StreamInternalError If any of the following occurs:
 *       - block registration fails
 *       - the multicast block cannot be initialized properly
 *  - ::NvSciError_InsufficientMemory Memory allocation fails.
 * \if NV_SAFETY
 *  - Panics if locking any internal mutex fails due to system level error.
 * \endif
 ...
 *  - ::NvSciError_StreamInternalError If any of the following occurs:
 *       - block registration fails
 *       - connection establishment through @a ipcEndpoint fails
 ...
 * @pre @id{NvSciStreamIpcSrcCreate_PreCond_002} @asil{QM}
 ...
 * \if NV_QNX
 *  - ::NvSciError_InvalidState API is called in runtime state.
 *  - ::NvSciError_NotSupported Inter-VM endpoint not supported in QNX.
 * \endif
 *  - ::NvSciError_StreamInternalError If any of the following occurs:
 *       - block registration fails
 *       - connection establishment through @a ipcEndpoint fails
 *       - IpcSrc or C2CSrc block cannot be initialized properly
 *       - default FIFO queue cannot be initialized properly when queue is not
 *         provided by user if the @a ipcEndpoint is a C2C endpoint.
 *  - ::NvSciError_BadParameter If any of the following occurs:
 *       - the output parameter @a ipc is a null pointer
 *       - the input parameter @a queue does not reference a queue block when
 *         the @a ipcEndpoint is a C2C backend type
 *       - the input parameter @a queue is valid when the @a ipcEndpoint is
 *         a non-C2C backend type
 *  - ::NvSciError_StreamBadBlock If the input parameter @a queue is provided
 *      but invalid when the @a ipcEndpoint is a C2C backend type
 ...
 *   NvSciIpcResetEndpointSafe() must be called on NvSciIpcEndpoint before
 *   passing it to NvSciStream.
 * @pre @id{NvSciStreamIpcSrcCreate2_PreCond_002} @asil{QM}
 *  Valid NvSciIpcEndpoint is obtained.
 *
 ...
 *       - block registration fails
 *       - connection establishment through @a ipcEndpoint fails
 *       - IpcDst block can't be initialized properly
 *  - ::NvSciError_BadParameter The output parameter @a ipc is a null pointer.
 *  - ::NvSciError_NotInitialized @a ipcEndpoint is uninitialized.
 *  - ::NvSciError_InsufficientMemory Memory allocation fails.
 * \if NV_SAFETY
 ...
 *   NvSciIpcResetEndpointSafe() must be called on NvSciIpcEndpoint before
 *   passing it to NvSciStream.
 * @pre @id{NvSciStreamIpcDstCreate_PreCond_002} @asil{QM}
 *  Valid NvSciIpcEndpoint is obtained.
 *
 ...
 * \if NV_QNX
 *  - ::NvSciError_InvalidState API is called in runtime state.
 *  - ::NvSciError_NotSupported Inter-VM backend endpoint not supported in QNX
 * \endif
 *  - ::NvSciError_StreamInternalError If any of the following occurs:
 *       - block registration fails
 *       - connection establishment through @a ipcEndpoint fails
 *       - IpcDst or C2CDst block cannot be initialized properly
 *  - ::NvSciError_BadParameter If any of the following occurs:
 *       - the output parameter @a ipc is a null pointer
 *       - the input parameter @a pool does not reference a pool block when the
 *         @a ipcEndpoint is a C2C backend type
 *       - the input parameter @a pool is valid when the @a ipcEndpoint
 *         is a non-C2C backend type
 *  - ::NvSciError_StreamBadBlock If the input parameter @a pool is provided
 *      but invalid when the @a ipcEndpoint is a C2C backend type
 *  - ::NvSciError_InsufficientResource If the pool block is already
 ...
 *   The NvSciIpcEndpoint should not be used by the application for any
 *   other purpose once provided to NvSciStream.
 * @pre @id{NvSciStreamIpcDstCreate2_REC_002} @asil{QM}
 *  Valid NvSciIpcEndpoint is obtained.
 *
 * @usage
 * - Allowed context for the API call
 ...
 *
 * - Creates a block to perform the CPU waiting operation on the fences for the
 *   packets received from consumer before sending them upstream.
 *
 ...
 *  - ::NvSciError_StreamInternalError If any of the following occurs:
 *       - block registration fails
 *       - the ReturnSync block can't be initialized properly
 *  - ::NvSciError_InsufficientMemory Memory allocation fails.
 * \if NV_SAFETY
 *  - Panics if locking any internal mutex fails due to system level error.
 * \endif
 ...
 *
 * - Creates a block to perform the CPU waiting operation on the fences for the
 *   packets received from producer before sending them downstream.
 *
 ...
 *  - ::NvSciError_StreamInternalError If any of the following occurs:
 *       - block registration fails
 *       - the PresentSync block can't be initialized properly
 *  - ::NvSciError_InsufficientMemory Memory allocation fails.
 * \if NV_SAFETY
 *  - Panics if locking any internal mutex fails due to system level error.
 * \endif
 ...
 *  - ::NvSciError_StreamNotConnected Stream is not fully connected, which means
 *      NvSciStreamEventType_Connected is not posted on the block or
 *      NvSciStreamEventType_Disconnect is posted.
 * \if NV_SAFETY
 *  - Panics if locking any internal mutex fails due to system level error.
 * \endif
 ...
 * \endif
 *  - ::NvSciError_StreamBadBlock Block referenced by @a block is invalid.
 *  - ::NvSciError_NotSupported Operation not supported on the block referenced
 *      by @a block. Only supported on producer, consumer, pool and multicast
 *      block.
 ...
 *      process the data.
 *  - ::NvSciError_StreamNotConnected Stream is not fully connected when it is
 *      called on producer, consumer or pool block, which means
 *      NvSciStreamEventType_Connected is not yet queried by the application
 *      by calling NvSciStreamBlockEventQuery() or
 *      NvSciStreamEventType_Disconnect is posted.
 * \if NV_SAFETY
 *  - Panics if locking any internal mutex fails due to system level error.
 * \endif
 ...
 *  - ::NvSciError_StreamBadBlock Block referenced by @a block is invalid.
 *  - ::NvSciError_NotSupported Operation not supported on the block referenced
 *      by @a block. Only supported on producer, consumer and pool block.
 ...
 *      element.
 *  - ::NvSciError_BadParameter If @a bufAttrList is NULL.
 *  - ::NvSciError_StreamNotConnected Stream is not fully connected, which means
 *      NvSciStreamEventType_Connected is not yet queried by the application
 *      by calling NvSciStreamBlockEventQuery() or NvSciStreamEventType_Disconnect
 *      is posted.
 ...
 *      by calling NvSciStreamBlockEventQuery() or NvSciStreamEventType_Disconnect
 *      is posted.
 *  - ::NvSciError_IndexOutOfRange @a elemIndex out of range.
 *  - Any error that NvSciBufAttrListClone() can generate.
 * \if NV_SAFETY
 *  - Panics if locking any internal mutex fails due to system level error.
 * \endif
 ...
 *   elements will NOT be visible to the consumer block at @a consumerIndex.
 ...
 *  - ::NvSciError_StreamBadBlock Block referenced by @a block is invalid.
 *  - ::NvSciError_NotSupported Operation not supported on the block referenced
 *      by @a block. Only supported on pool block.
 *  - ::NvSciError_NotYetAvailable The requested element information has
 *      not yet arrived.
 *  - ::NvSciError_NoLongerAvailable The element export phase was completed.
 *  - ::NvSciError_BadParameter The @a consumerIndex is out of range or
 *      entry with @a userType not found in the allocated element list.
 *  - ::NvSciError_StreamInternalError Inconsistency in internal data structures.
 *  - ::NvSciError_StreamNotConnected Stream is not fully connected, which means
 *      NvSciStreamEventType_Connected is not yet queried by the application
 *      by calling NvSciStreamBlockEventQuery() or NvSciStreamEventType_Disconnect
 *      is posted.
 ...
 *      currently interacting with the block's imported element information.
 *      The call can be retried.
 *  - ::NvSciError_IndexOutOfRange @a consumerIndex out of range.
 * \if NV_SAFETY
 *  - Panics if locking any internal mutex fails due to system level error.
 * \endif
 ...
 *  - ::NvSciError_StreamBadBlock Block referenced by @a block is invalid.
 *  - ::NvSciError_NotSupported Operation not supported on the block referenced
 *      by @a block. Only supported on consumer block.
 *  - ::NvSciError_NotYetAvailable The requested element information has
 *      not yet arrived.
 *  - ::NvSciError_NoLongerAvailable The element import phase was completed.
 ...
 *  - ::NvSciError_InvalidState API is called in runtime state.
 * \endif
 *  - ::NvSciError_StreamBadBlock Block referenced by @a pool is invalid.
 *  - ::NvSciError_NotSupported Operation not supported on the block referenced
 *      by @a pool. Only supported on pool block.
 ...
 *  - ::NvSciError_NotSupported Operation not supported on the block referenced
 *      by @a pool. Only supported on pool block.
 *  - ::NvSciError_StreamBadPacket Packet referenced by @a handle is invalid.
 ...
 *  - ::NvSciError_StreamBadBlock Block referenced by @a block is invalid.
 *  - ::NvSciError_NotSupported Operation not supported on the block referenced
 *      by @a block. Only supported on producer and consumer block.
 *  - ::NvSciError_BadAddress @a handle is NULL.
 ...
 *      by calling NvSciStreamBlockEventQuery() or NvSciStreamEventType_Disconnect
 *      is posted.
 *  - Any error or panic behavior returned by NvSciBufObjDup(), NvSciBufObjRef().
 * \if NV_SAFETY
 *  - Panics if locking any internal mutex fails due to system level error.
 * \endif
 ...
 *      by calling NvSciStreamBlockEventQuery() or NvSciStreamEventType_Disconnect
 *      is posted.
 *  - ::NvSciError_BadParameter: @a queryBlockType does not indicate
 *      producer or consumer endpoint.
 * \if NV_SAFETY
 *  - Panics if locking any internal mutex fails due to system level error.
 * \endif
...
/*!
 * \brief Specifies a user-defined @a userType and a list of NvSciBuf
 *   attributes for an element along with corresponding NvSciSync waiter
 *   attributes in a single call.
 *
 * This API allows endpoints to set both element and waiter attributes
 * together, reducing setup overhead and ensuring consistency between
 * element and sync attributes.
 *
 * The application is responsible for indicating which elements it will use
 *   by adding them before setup phase completion via
 *   NvSciStreamBlockSetupStatusSet() with
 *   NvSciStreamSetup_ElementAndWaiterAttrsExport. Unused entries will be
 *   treated as if they are unsupported.
 *
 * This API is only supported on producer and consumer blocks.
 *
 * It is mutually
 *   exclusive with the separate NvSciStreamBlockElementAttrSet() and
 *   NvSciStreamBlockElementWaiterAttrSet() APIs. If any endpoint in a
 *   stream uses this API, all endpoints must use it.
 *
 * <b>Actions</b>
 * - Saves both the element attribute list and waiter sync attribute list.
 * - Increments the element count.
 *
 * \param[in] block: NvSciStreamBlock which references a block.
 * \param[in] userType: User-defined element type.
 * \param[in] bufAttrList: NvSciBuf attribute list for the element.
 * \param[in] waitSyncAttrList: NvSciSync waiter attribute list.
 *
 * \return ::NvSciError, the completion code of this operation.
 *  - ::NvSciError_Success Operation completed successfully.
 * \if NV_QNX
 ...
 *  - ::NvSciError_StreamBadBlock Block referenced by @a block is invalid.
 *  - ::NvSciError_NotSupported Operation not supported on the block referenced
 *      by @a block. Only supported on producer and consumer blocks. Also
 *      returned if this API is mixed with separate element/waiter APIs.
 *  - ::NvSciError_BadParameter @a bufAttrList is NULL or @a waitSyncAttrList
 *      is NULL or @a userType is 0 (reserved).
 *  - ::NvSciError_NoLongerAvailable The element and waiter attr export phase
 *      was completed.
 *  - ::NvSciError_StreamNotConnected Stream is not fully connected, which means
 *      NvSciStreamEventType_Connected is not yet queried by the application
 *      by calling NvSciStreamBlockEventQuery() or NvSciStreamEventType_Disconnect
 *      is posted.
 *  - ::NvSciError_InsufficientMemory Not enough system memory available to
 *      allocate internal structures.
 *  - Any error or panic behavior that NvSciBufAttrListClone() can generate.
 *  - Any error or panic behavior that NvSciSyncAttrListClone() can generate.
 * \if NV_SAFETY
 *  - Panics if locking any internal mutex fails due to system level error.
 * \endif
 *
 * @pre @id{NvSciStreamBlockElementAndWaiterAttrSet_PreCond_001} @asil{QM}
 *   The block must be a producer or consumer block in the phase where
 *   combined element and waiter export is available. This phase begins after
 *   receiving the NvSciStreamEventType_Connected event and ends after a call
 *   to NvSciStreamBlockSetupStatusSet() with
 *   NvSciStreamSetup_ElementAndWaiterAttrsExport.
 *
 * @pre @id{NvSciStreamBlockElementAndWaiterAttrSet_PreCond_002} @asil{QM}
 *   If this API is used by any endpoint in the stream, all producer and
 *   consumer endpoints must use this API instead of the separate
 *   NvSciStreamBlockElementAttrSet() and NvSciStreamBlockElementWaiterAttrSet().
 *
 * @post
 *   None
 *
 ...
 */
NvSciError
NvSciStreamBlockElementAndWaiterAttrSet(
    NvSciStreamBlock const block,
    uint32_t const userType,
    NvSciBufAttrList const bufAttrList,
    NvSciSyncAttrList const waitSyncAttrList
 ...
 *      by calling NvSciStreamBlockEventQuery() or NvSciStreamEventType_Disconnect
 *      is posted.
 *  - Any error or panic behavior that NvSciSyncObjGetAttrList(),
 *      NvSciSyncAttrListValidateReconciled() or NvSciSyncObjRef() can generate.
 * \if NV_SAFETY
 *  - Panics if locking any internal mutex fails due to system level error.
 * \endif
 ...
 *      by calling NvSciStreamBlockEventQuery() or NvSciStreamEventType_Disconnect
 *      is posted.
 *  - Any error or panic behavior that NvSciSyncObjRef(), NvSciSyncObjDup()
 *     can generate.
 * \if NV_SAFETY
 *  - Panics if locking any internal mutex fails due to system level error.
 * \endif
 ...
 *  - ::NvSciError_StreamBadBlock Block referenced by @a producer is invalid.
 *  - ::NvSciError_NotSupported Operation not supported on the block referenced
 *      by @a producer. Only supported on producer block.
 *  - ::NvSciError_BadAddress @a cookie is NULL.
 ...
 *  - ::NvSciError_StreamBadBlock Block referenced by @a consumer is invalid.
 *  - ::NvSciError_NotSupported Operation not supported on the block referenced
 *      by @a consumer. Only supported on consumer block.
 *  - ::NvSciError_BadAddress @a cookie is NULL.
 *  - ::NvSciError_NoStreamPacket No packet is available.
 *  - ::NvSciError_StreamInternalError Inconsistency in internal data structures.
 *  - ::NvSciError_StreamNotConnected Stream is not fully connected, which means
 ...
 *  - ::NvSciError_StreamPacketInaccessible @a handle is not currently
 *      held by the application.
 *  - ::NvSciError_StreamNotConnected When called on consumer block, before the
 *.     NvSciStreamEventType_Connected event has been queried by calling the
 ...
 *      @a block.
 *  - ::NvSciError_StreamNotConnected Stream is not fully connected when it is
 *      called on a producer block, which means NvSciStreamEventType_Connected is
 *      not yet queried by the application by calling NvSciStreamBlockEventQuery()
 *      or NvSciStreamEventType_Disconnect is posted.
 ...
 *  - ::NvSciError_StreamBadBlock Block referenced by @a block is invalid.
 *  - ::NvSciError_NotSupported Operation not supported on the block referenced
 *      by @a block. Only supported on ipcsrc block.
 * \if NV_SAFETY
 *  - Panics if locking any internal mutex fails due to system level error.
 * \endif
 ...
 *       - API is called in runtime state.
 * \endif
 *       - An NvSciStream API has already been called on the block referenced
 *         by @a block.
 *  - ::NvSciError_BadParameter If @a eventService or @a eventNotifier is null
 *  - ::NvSciError_StreamBadBlock If block referenced by @a block is invalid.
 *  - Any error or panic behavior that NvSciEventService::CreateLocalEvent()
 ...
 *   return error.
 * \if NV_SAFETY
 * @post @id{NvSciStreamBlockEventServiceSetup_PostCond_002}
 *   If there are any failures to report the occurrence of any events on the
 *   @a block, through @a eventNotifier, then NvSciStream will panic.
 * \endif
 ...
 * \endif
 *  - ::NvSciError_NotSupported Operation not supported on the block referenced
 *      by @a block. Only supported on ipcsrc and ipcdst block.
 *  - ::NvSciError_AlreadyDone Internal event service setup done.
 *  - ::NvSciError_BadParameter If any of the following occurs:
 *       - @a eventService, @a notifierCount or @a eventNotifierArray is null
 *         or invalid
 *  - ::NvSciError_StreamBadBlock If block referenced by @a block is invalid.
 *  - ::NvSciError_InsufficientMemory Size of the input array @a notifierCount
 *      not large enough.
 *  - Any error or panic behavior that following APIs can generate:
 *    NvSciIpcBindEventService(), NvSciIpcGetEventNotifier(),
 *    NvSciEventNotifier::SetHandler(), NvSciEventService::CreateLocalEvent().
 * \if NV_SAFETY
 *  - Panics if locking any internal mutex fails due to system level error.
 * \endif
 ...
 * @pre @id{NvSciStreamBlockUserInfoSet_PreCond_002} @asil{B}
 *   dataSize parameter should match the size of the input data array.
 *
 * @post
 *   None
 *
 ...
 * \endif
 *  - ::NvSciError_StreamBadBlock Block referenced by @a block is invalid.
 *  - ::NvSciError_BadAddress If any of the following occurs:
 *       - @a dataSize is NULL
 *       - @a data is NULL when @a dataSize is non-zero
 ...
 *  - ::NvSciError_NoLongerAvailable The configuration of the block instance
 *      is already done.
 *  - ::NvSciError_BadAddress @a crc or @a data is NULL.
 * \if NV_SAFETY
 *  - Panics if locking any internal mutex fails due to system level error.
 * \endif
 * @pre @id{NvSciStreamBlockUserInfoSetWithCrc_PreCond_001} @asil{QM}
 *   The block is created but not connected to any other block.
 * @pre @id{NvSciStreamBlockUserInfoSetWithCrc_PreCond_002} @asil{D}
 *   dataSize parameter should match the size of the input data array.
 *
 * @post
 *   None
 ...
 *  - ::NvSciError_StreamBadBlock Block referenced by @a block is invalid.
 *  - ::NvSciError_NotSupported Operation not supported on the block referenced
 *      by @a block. Only supported on producer, consumer and primary pool block.
 *  - ::NvSciError_NotYetAvailable The prerequisite event has not yet arrived.
 *  - ::NvSciError_NoLongerAvailable The element export phase was completed.
 *  - ::NvSciError_Busy An NvSciStream interface in another thread is
 *      currently interacting with the block's exported element information.
 ...
 *      by calling NvSciStreamBlockEventQuery() or NvSciStreamEventType_Disconnect
 *      is posted.
 *  - ::NvSciError_BadAddress @a crc pointer is NULL.
 *  - Any error that NvSciBufAttrListClone() can generate.
 * \if NV_SAFETY
 *  - Panics if locking any internal mutex fails due to system level error.
 * \endif
 ...
 *      is posted.
 *  - ::NvSciError_BadAddress @a crc pointer is NULL.
 *  - Any error or panic behavior returned by NvSciBufObjGetId()
 *    or NvSciBufObjRef().
 * \if NV_SAFETY
 *  - Panics if locking any internal mutex fails due to system level error.
 * \endif
 ...
 *      is posted.
 *  - ::NvSciError_BadAddress @a crc pointer is NULL.
 *  - Any error or panic behavior returned by NvSciBufObjGetId(), NvSciBufObjDup(),
 *    NvSciBufObjRef().
 * \if NV_SAFETY
 *  - Panics if locking any internal mutex fails due to system level error.
 * \endif
 ...
 *   Block must not have rejected the packet, and must still be in packet import phase.
 * @pre @id{NvSciStreamBlockPacketBufferGetWithCrc_PreCond_002} @asil{D}
 *   crcCount parameter should match the size of the input crc array.
 *
 * @post
 *   None
 *
 ...
 *      is posted.
 *  - ::NvSciError_BadAddress @a crc pointer is NULL.
 *  - Any error or panic behavior that NvSciSyncObjIdGet(), NvSciSyncObjGetAttrList()
 *    NvSciSyncAttrListValidateReconciled() or NvSciSyncObjRef() can generate.
 * \if NV_SAFETY
 *  - Panics if locking any internal mutex fails due to system level error.
 * \endif
 ...
 *      is posted.
 *  - ::NvSciError_BadAddress @a crc pointer is NULL.
 *  - Any error or panic behavior that NvSciSyncObjIdGet(), NvSciSyncObjRef(),
 *    NvSciSyncObjDup() can generate.
 * \if NV_SAFETY
 *  - Panics if locking any internal mutex fails due to system level error.
 * \endif
 ...
 *      @a block.
 *  - ::NvSciError_StreamNotConnected Stream is not fully connected when it is
 *      called on a producer block, which means NvSciStreamEventType_Connected
 *      is not yet queried by the application by calling
 *      NvSciStreamBlockEventQuery() or NvSciStreamEventType_Disconnect is
 *      posted.
 *  - ::NvSciError_BadAddress @a crc pointer is NULL.
 *  - Any error returned by NvSciSyncFenceCrcGet() or NvSciSyncFenceDup().
 * \if NV_SAFETY
 *  - Panics if locking any internal mutex fails due to system level error.
 * \endif
 ...
 *      posted.
 *  - ::NvSciError_BadAddress @a crc pointer is NULL.
 *  - Any error returned by NvSciSyncFenceCrcGet(), NvSciSyncFenceDup()
 *    or NvSciSyncIpcImportFence().
 * \if NV_SAFETY
 *  - Panics if locking any internal mutex fails due to system level error.
 * \endif
...
/*!
 * \brief Configure the block according to the @a streamConfigArr before
 *    connection.
 *
 * <b>Actions</b>
 * - None
 *
 * \param[in] block NvSciStreamBlock which references a block.
 * \param[in] streamConfigArr: Array of NvSciStreamConfig.
 * \param[in] cfgArrCount: Number of NvSciStreamConfig pairs.
 *
 * \return ::NvSciError, the completion code of this operation.
 *  - ::NvSciError_Success Operation completed successfully.
 * \if NV_QNX
 *  - ::NvSciError_InvalidState if API is called in runtime state.
 * \endif
 *  - ::NvSciError_StreamBadBlock Block referenced by @a block is invalid.
 *  - ::NvSciError_NotSupported If any of the following occurs:
 *      - Operation not supported on the block referenced by @a block
 *      - NvSciStreamConfigKey not supported on the block referenced by @a block
 *  - ::NvSciError_NoLongerAvailable Block configuration is finalized once
 *      connected with other block.
 *  - ::NvSciError_BadParameter If any of the following occurs:
 *      - @a cfgArrCount is zero
 *      - @a streamConfigArr is NULl
 *      - The config value length doesn't match
 *      - The config value is NULL.
 * \if NV_SAFETY
 *  - Panics if locking any internal mutex fails due to system level error.
 * \endif
 * @pre @id{NvSciStreamBlockConfig_PreCond_001} @asil{QM}
 *   This API should be called after the input block is created and before
 *   connecting it to other block.
 * @pre @id{NvSciStreamBlockConfig_PreCond_002} @asil{D}
 *   cfgArrCount parameter should match the size of the input streamConfigArr.
 *
 * @post
 *   None
 *
 ...
 */
NvSciError
NvSciStreamBlockConfig(
    NvSciStreamBlock const block,
    NvSciStreamConfig const* const streamConfigArr,
    uint32_t const cfgArrCount
);
